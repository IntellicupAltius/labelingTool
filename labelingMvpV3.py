#!/usr/bin/env python3
"""
TakeLabeler v6.4 — Export ALL labeled frames + Speed slider

Adds:
- Export all labeled frames (File menu + top-bar button)
- Speed slider (1..5): higher = faster (more frames/tick, shorter delay)

Keeps from v6.3:
- HEVC-friendly _safe_seek
- Start/End buttons (and Home/End hotkeys)
- P = play/pause, Shift+Left/Right = -50/+50
- Freeze on last frame with popup
- Per-frame annotation memory; right sidebar (current-frame + all)
"""
...
class TakeLabeler(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("TakeLabeler v6.4 — Video Labeler for YOLOv8")
        ...
        # Speed params (set by slider)
        self.speed_var = tk.IntVar(value=3)  # 1..5
    ...
    def _build_ui(self):
        ...
        # Speed slider (1..5). Higher = faster
        ttk.Label(mid, text="Speed").pack(side=tk.LEFT)
        speed_scale = ttk.Scale(mid, from_=1, to=5, orient="horizontal",
        ...
        filemenu.add_command(label="Export ALL labeled frames", command=self._export_all_labeled)
    ...
    def _apply_speed(self, s: int):
        # Map 1..5 -> (play_step, play_delay_ms)
        # Feel free to tune these mappings.
        mapping = {
            1: (1, 35),
            2: (2, 30),
            3: (3, 25),
        ...
        self.play_step, self.play_delay_ms = mapping.get(int(s), (3, 25))
    ...
    def _show_quick_start(self):
        self.after(400, lambda: messagebox.showinfo(
            ...
            "Use the Speed slider to adjust playback.\n"
    ...
    def show_help(self):
        messagebox.showinfo("Help",
            ...
            "Speed slider: higher = faster playback (more frames per tick).\n"
            "Boxes are hidden while playing, shown when paused on their frame."
        )
    ...
    def _show_frame(self, frame_bgr, draw_boxes: bool):
        ...
        if self.fps > 0:
            ...
            mm, ss = 0, 0
        self.status.set(f"{self.video_path.name if self.video_path else ''} | Frame {self.frame_idx+1} | {mm:02d}:{ss:02d} | speed {self.play_step}x")
    ...
    def _export_all_labeled(self):
        ...
        # (subdirs per model are created on demand below)
        ...
        for f in sorted(self.ann_by_frame.keys()):
            ...
            # Seek safely to exact frame, decode it, and use current_frame_rgb
            if not self._safe_seek(f):
            ...
            # Group by model so each model gets its own YOLO txt
            by_model: Dict[str, List[Annotation]] = {}
            for ann in anns:
                by_model.setdefault(ann.model, []).append(ann)
