#!/usr/bin/env python3
"""
TakeLabeler v6.9 — Entry + Listbox for Class selection (no focus loss!)

- Model combobox is readonly
- Class uses Entry + always-visible Listbox for filtering
- Type to filter, Up/Down to navigate, Enter to Save
...
class TakeLabeler(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Yolo Labeler v6.9 — Video Labeler for YOLOv8")
        ...
        self._last_annotation: Optional[Tuple[str, str]] = None
    ...
    def _build_ui(self):
        ...
        self.btn_end.pack(side=tk.LEFT, padx=(2, 8))
        ...
        sidebar = ttk.Frame(main, width=360);
        ...
        ttk.Button(sidebar, text="Delete selected (current frame)", command=self.delete_selected_ann_current).pack(
        ...
        ttk.Button(btns, text="Go to selected", command=self.goto_selected_global).pack(side=tk.LEFT, expand=True,
                                                                                        fill=tk.X, padx=(0, 4))
        ttk.Button(btns, text="Delete selected", command=self.delete_selected_global).pack(side=tk.LEFT, expand=True,
                                                                                           fill=tk.X, padx=(4, 0))
    ...
    def _apply_speed(self, s: int):
        mapping = {
            1: (1, 35), 2: (2, 30), 3: (4, 25), 4: (6, 22),
            5: (8, 20), 6: (10, 18), 7: (12, 16), 8: (16, 14),
        }
    ...
    def _show_quick_start(self):
        self.after(400, lambda: messagebox.showinfo(
            ...
            "Draw a box → popup for Model/Class → Type to filter → Enter to Save.\n"
    ...
    def show_help(self):
        messagebox.showinfo("Help",
                            ...
                            "Assign dialog: Type to filter classes, Up/Down to navigate, Enter = Save"
                            )
    ...
    def on_load_models(self):
        ...
        if hasattr(self, '_last_loaded_model'):
            pass
        else:
            models = list(self.model_to_names.keys())
            if models:
                self._last_loaded_model = models[0]
        self._update_transport_enabled()
    ...
    def on_load_one_model(self):
        ...
        self._last_loaded_model = chosen.stem
        self._update_transport_enabled()
    ...
    def _open_fixed_folder_picker(self, title="Select a folder", start_dir: Optional[Path] = None) -> Optional[Path]:
        ...
        b_back.pack(side=tk.LEFT, padx=(0, 5));
        b_fwd.pack(side=tk.LEFT, padx=(0, 10))
        b_up.pack(side=tk.LEFT, padx=(0, 5));
        b_home.pack(side=tk.LEFT, padx=(0, 10))
        ...
        lb_dirs = tk.Listbox(mid, height=20);
        lb_dirs.pack(fill=tk.BOTH, expand=True, pady=(4, 0))

        bot = ttk.Frame(dlg);
        bot.pack(side=tk.BOTTOM, fill=tk.X, padx=10, pady=10)
        b_select = ttk.Button(bot, text="Select", width=12)
        b_cancel = ttk.Button(bot, text="Cancel", width=12)
        b_select.pack(side=tk.RIGHT, padx=(8, 0));
        ...
        def go(newdir: Path, add_hist=True):
            if newdir.exists() and newdir.is_dir():
                ...
                if add_hist:
                    del history[hpos[0] + 1:];
                    history.append(cur_dir[0]);
                    hpos[0] = len(history) - 1
                refresh()
        ...
        def on_cancel():
            chosen["dir"] = None; close()

        b_back.configure(command=on_back);
        b_fwd.configure(command=on_fwd)
        b_up.configure(command=on_up);
        b_home.configure(command=on_home)
        b_select.configure(command=on_select);
        b_cancel.configure(command=on_cancel)
        lb_dirs.bind("<Double-Button-1>", on_dir_double)
        ent_path.bind("<Return>", on_enter_path);
        ent_path.bind("<KP_Enter>", on_enter_path)
    ...
    def _open_fixed_yaml_picker(self, title="Select a YOLO data.yaml", start_dir: Optional[Path] = None) -> Optional[
        Path]:
        start_dir = Path(start_dir or getattr(self, "_model_last_dir", str(Path.home()))).resolve()
        dlg = tk.Toplevel(self)
        dlg.title(title);
        ...
        lb_dirs = tk.Listbox(left, height=20);
        lb_dirs.pack(fill=tk.BOTH, expand=True, pady=(4, 0))

        right = ttk.Frame(mid);
        right.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(10, 0))
        ttk.Label(right, text="data.yaml / .yml").pack(anchor="w")
        lb_files = tk.Listbox(right, height=20);
        lb_files.pack(fill=tk.BOTH, expand=True, pady=(4, 0))

        bot = ttk.Frame(dlg);
        bot.pack(side=tk.BOTTOM, fill=tk.X, padx=10, pady=10)
        b_open = ttk.Button(bot, text="Open", width=12)
        b_cancel = ttk.Button(bot, text="Cancel", width=12)
        b_open.pack(side=tk.RIGHT, padx=(8, 0));
        b_cancel.pack(side=tk.RIGHT)
        ...
        def on_open():
            sel = lb_files.curselection()
            if not sel:
                messagebox.showinfo("Pick a model", "Select a data.yaml file from the list.");
                return
            chosen["file"] = cur_dir[0] / lb_files.get(sel[0]);
            close_ok()
        ...
        b_up.configure(command=on_up);
        b_home.configure(command=on_home)
        b_open.configure(command=on_open);
        b_cancel.configure(command=on_cancel)
        lb_dirs.bind("<Double-Button-1>", on_dir_double)
    ...
    def _open_fixed_video_picker(self, title="Select a video file") -> Optional[Path]:
        ...
        frm_mid = ttk.Frame(dlg);
        frm_mid.pack(side=tk.TOP, fill=tk.BOTH, expand=True, padx=10, pady=(0, 8))
        frm_left = ttk.Frame(frm_mid, width=320);
        frm_left.pack(side=tk.LEFT, fill=tk.Y)
        ttk.Label(frm_left, text="Folders").pack(anchor="w")
        lb_dirs = tk.Listbox(frm_left, height=20);
        lb_dirs.pack(fill=tk.BOTH, expand=True, pady=(4, 0))

        frm_right = ttk.Frame(frm_mid);
        frm_right.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(10, 0))
        ttk.Label(frm_right, text="Video files").pack(anchor="w")
        lb_files = tk.Listbox(frm_right, height=20);
        lb_files.pack(fill=tk.BOTH, expand=True, pady=(4, 0))

        frm_bot = ttk.Frame(dlg);
        frm_bot.pack(side=tk.BOTTOM, fill=tk.X, padx=10, pady=10)
        ...
        def on_dir_double(_evt=None):
            sel = lb_dirs.curselection()
            if not sel: return
            name = lb_dirs.get(sel[0]);
            go_dir(cur_dir[0] / name)

        def on_file_double(_evt=None):
            sel = lb_files.curselection()
            if not sel: return
            name = lb_files.get(sel[0]);
            selection["file"] = cur_dir[0] / name;
            close_ok()
        ...
        btn_fwd.configure(command=on_fwd)
        btn_up.configure(command=on_up);
        btn_home.configure(command=on_home)
        btn_open.configure(command=on_open);
        ...
        if chosen:
            self._video_last_dir = str(chosen.parent)
        else:
            self._video_last_dir = str(cur_dir[0])
        return chosen
    ...
    def on_load_video(self):
        ...
        cap = cv2.VideoCapture(str(self.video_path), cv2.CAP_FFMPEG) if hasattr(cv2,
                                                                                "CAP_FFMPEG") else cv2.VideoCapture(
            str(self.video_path))
        if not cap or not cap.isOpened():
            cap = cv2.VideoCapture(str(self.video_path))
            if not cap.isOpened():
                messagebox.showerror("Failed", f"Could not open video: {self.video_path}");
                return
        self.cap = cap
    ...
    def _update_transport_enabled(self):
        ...
        self.btn_end.configure(state=state)
    ...
    def toggle_play(self):
        ...
        if not self.model_to_names:
            messagebox.showinfo("Models required", "Load the Models Folder before playing.");
            return
        if self.end_reached:
            messagebox.showinfo("End of video", "Reached the end.");
            return
        if self.playing:
            self.playing = False;
            self.play_btn.configure(text="Play [P]")
            ...
            self._cancel_play_timer()
            self.playing = True;
            self.play_btn.configure(text="Pause [P]")
            self._play_loop()
    ...
    def step_seek(self, delta: int):
        ...
        if not self.model_to_names:
            messagebox.showinfo("Models required", "Load the Models Folder before stepping.");
            return
    ...
    # ---------- Assign dialog with Entry + Listbox (no focus loss!) ----------
    def _open_assign_dialog(self, x1, y1, x2, y2):
        if not self.model_to_names:
            messagebox.showinfo("Models required", "Load the Models Folder first.")
            return

        dlg = tk.Toplevel(self)
        dlg.title("Assign label")
        dlg.transient(self)
        dlg.grab_set()
        dlg.geometry("420x500")

        # Model dropdown (običan combobox - readonly)
        ttk.Label(dlg, text="Model:").grid(row=0, column=0, padx=8, pady=8, sticky="e")
        model_var = tk.StringVar(value="")
        model_combo = ttk.Combobox(dlg, textvariable=model_var, width=28, state="readonly")
        model_combo.grid(row=0, column=1, padx=8, pady=8, sticky="w")
        model_combo["values"] = list(self.model_to_names.keys())

        # Class - Entry + Listbox (umesto comboboxa)
        ttk.Label(dlg, text="Class (type to filter):").grid(row=1, column=0, padx=8, pady=(8, 4), sticky="ne")

        class_frame = ttk.Frame(dlg)
        class_frame.grid(row=1, column=1, padx=8, pady=(8, 4), sticky="ew")

        class_var = tk.StringVar(value="")
        class_entry = ttk.Entry(class_frame, textvariable=class_var, width=30)
        class_entry.pack(fill=tk.X)

        # Listbox sa filteriranim rezultatima - UVEK VIDLJIV
        class_listbox = tk.Listbox(class_frame, height=14, width=30)
        class_listbox.pack(fill=tk.BOTH, expand=True, pady=(4, 0))

        all_classes = []

        def refresh_classes(*_):
            nonlocal all_classes
            all_classes = self.model_to_names.get(model_var.get(), [])
            filter_and_update_list()

        def filter_and_update_list(*_):
            """Filtrira i ažurira Listbox na osnovu unetog teksta - ignoriše razmake, crtice i donje crte"""
            filter_text = class_var.get().lower()
            class_listbox.delete(0, tk.END)

            # Normalizuj filter text - ukloni razmake, crtice i donje crte
            normalized_filter = filter_text.replace(" ", "").replace("-", "").replace("_", "")

            if not normalized_filter:
                # Ako nema filtera, prikaži sve
                for cls in all_classes:
                    class_listbox.insert(tk.END, cls)
            else:
                # Filtriraj - normalizuj i klase i filter
                filtered = []
                for cls in all_classes:
                    normalized_cls = cls.lower().replace(" ", "").replace("-", "").replace("_", "")
                    if normalized_filter in normalized_cls:
                        filtered.append(cls)

                for cls in filtered:
                    class_listbox.insert(tk.END, cls)

            # Auto-select prvi rezultat
            if class_listbox.size() > 0:
                class_listbox.selection_clear(0, tk.END)
                class_listbox.selection_set(0)
                class_listbox.see(0)

        # Bind za Entry - filter dok kucaš
        class_var.trace_add("write", filter_and_update_list)

        # Bind za Listbox - klik na item ga prebacuje u Entry
        def on_listbox_select(_evt=None):
            sel = class_listbox.curselection()
            if sel:
                selected_class = class_listbox.get(sel[0])
                class_var.set(selected_class)
                class_entry.icursor(tk.END)
                class_entry.focus_set()

        class_listbox.bind("<<ListboxSelect>>", on_listbox_select)
        class_listbox.bind("<Double-Button-1>", lambda e: do_save())

        # Strelice gore/dole u Entry-ju navigiraju kroz Listbox
        def on_entry_key(e):
            if e.keysym == "Down":
                if class_listbox.size() > 0:
                    current = class_listbox.curselection()
                    if not current:
                        class_listbox.selection_set(0)
                    elif current[0] < class_listbox.size() - 1:
                        class_listbox.selection_clear(current[0])
                        class_listbox.selection_set(current[0] + 1)
                        class_listbox.see(current[0] + 1)
                return "break"
        ...
        class_entry.bind("<KeyPress>", on_entry_key)

        # Ako postoji prethodno učitan model – automatski ga izaberi
        if hasattr(self, "_last_loaded_model") and self._last_loaded_model in self.model_to_names:
            model_var.set(self._last_loaded_model)
            refresh_classes()

        model_combo.bind("<<ComboboxSelected>>", refresh_classes)
        refresh_classes()

        # Ako postoji poslednja anotacija, popuni je
        if self._last_annotation:
            last_model, last_class = self._last_annotation
            if last_model in self.model_to_names:
                model_var.set(last_model)
                refresh_classes()
                if last_class in self.model_to_names[last_model]:
                    class_var.set(last_class)
                    filter_and_update_list()
        ...
        def do_save(_evt=None):
            if not model_var.get():
                messagebox.showerror("Model", "Pick a model.")
                return
            names = self.model_to_names.get(model_var.get(), [])

            selected_class = class_var.get()
            if not selected_class or selected_class not in names:
                messagebox.showerror("Class", "Pick a valid class.")
                return

            class_id = names.index(selected_class)
            ann = Annotation(model=model_var.get(), class_name=selected_class, class_id=class_id,
                             x1=x1, y1=y1, x2=x2, y2=y2)
            self.annotations.append(ann)
            self._render_from_cache(draw_boxes=True)
            self._refresh_lists()
            self._last_annotation = (model_var.get(), selected_class)

            dlg.destroy()

        ttk.Button(btns, text="Save (Enter)", command=do_save, width=14).pack(side=tk.LEFT, padx=6)
        ttk.Button(btns, text="Cancel (Esc)", command=dlg.destroy, width=14).pack(side=tk.LEFT, padx=6)
    ...
    def on_mouse_down(self, evt):
        if not self.cap: return
        if not self.model_to_names:
            messagebox.showinfo("Models required", "Load the Models Folder before labeling.");
            return
        if self.playing: self.toggle_play()
        self.dragging = True
    ...
    def on_mouse_drag(self, evt):
        ...
        x2 = clamp(x2, 0, self.img_w - 1);
        y2 = clamp(y2, 0, self.img_h - 1)
    ...
    def _refresh_lists(self):
        ...
        for i, ann in enumerate(self.annotations, start=1):
            self.ann_list_frame.insert(tk.END,
                                       f"{i}. {ann.model}/{ann.class_name} [{ann.x1},{ann.y1}]→[{ann.x2},{ann.y2}]")
    ...
    def _rebuild_global_index(self):
        ...
        for f in sorted(self.ann_by_frame.keys()):
            for j, ann in enumerate(self.ann_by_frame[f]):
                ...
                self.ann_list_all.insert(tk.END,
                                         f"f{f:06d}  {ann.model}/{ann.class_name}  [{ann.x1},{ann.y1}]→[{ann.x2},{ann.y2}]")
    ...
    def _export_all_labeled(self):
        ...
        if not self.ann_by_frame:
            messagebox.showinfo("Nothing to export", "No labeled frames yet.");
            return
        ...
        # Kreiraj root output direktorijum
        try:
            ensure_dir(self.output_root)
        except Exception as e:
            messagebox.showerror("Export failed", f"Cannot create output directory:\n{e}")
            return
        ...
        for f in sorted(self.ann_by_frame.keys()):
            ...
            print(f"Processing frame {f} with {len(anns)} annotations")
            ...
            try:
                ...
                if not self._safe_seek(f):
                    ...
                    if not ok or frame is None:
                        failures += 1
                        error_messages.append(f"Frame {f}: Cannot read frame")
                        continue
                ...
                video_stem = video_stem.split("{")[0].rstrip("_-")  # ukloni UUID ako postoji
                base = f"{video_stem}_f{f + 1}"  # broji od 1, bez nula paddinga
                ...
                for model, model_anns in by_model.items():
                    try:
                        ...
                        # Proveri da li frame_rgb postoji i ima podatke
                        if frame_rgb is None or frame_rgb.size == 0:
                            error_messages.append(f"Frame {f}: Empty frame data")
                            continue

                        # Sačuvaj sliku
                        frame_bgr = cv2.cvtColor(frame_rgb, cv2.COLOR_RGB2BGR)
                        success = cv2.imwrite(str(img_out), frame_bgr)
                        print(f"  Wrote image: {img_out} (success={success})")

                        if not success:
                            error_messages.append(f"Frame {f}: cv2.imwrite failed for {img_out}")
                            continue

                        # Sačuvaj labels
                        with open(txt_out, "w", encoding="utf-8") as ftxt:
                            for a in model_anns:
                                line = yolo_line(a.class_id, a.x1, a.y1, a.x2, a.y2, self.img_w, self.img_h)
                                ftxt.write(line + "\n")
                            print(f"  Wrote labels: {txt_out}")

                    except Exception as e:
                        error_messages.append(f"Frame {f}, Model {model}: {str(e)}")
                        failures += 1
                        continue
                ...
                error_messages.append(f"Frame {f}: {str(e)}")
                failures += 1
                continue

        # Prikaz rezultata
        msg = f"Wrote {written_count} labeled frame(s) into:\n{self.output_root}\n"
        if failures > 0:
            msg += f"\nSkipped {failures} frame(s) due to errors."
            if error_messages:
