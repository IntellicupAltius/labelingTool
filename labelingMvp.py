#!/usr/bin/env python3
"""
TakeLabeler v6.1 — smooth playback, per-frame memory, global nav

Fixes vs your v6:
- Adds missing _redraw_boxes() (fixes AttributeError)
- No “rewind” at end-of-video: freeze on last decodable frame cleanly
- Status bar: no total frame count (shows Frame N and mm:ss)
- Current-frame list repopulates when you return to a labeled frame
- Right sidebar: current frame + all annotations (double-click to jump)
- ±50 stepping unchanged
"""
...
class TakeLabeler(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("TakeLabeler v6.1 — Video Labeler for YOLOv8")
    ...
    def _build_ui(self):
        ...
        self.play_btn = ttk.Button(mid, text="Play [P]", command=self.toggle_play, width=16)
    ...
    def _show_quick_start(self):
        self.after(400, lambda: messagebox.showinfo(
            ...
            "P = play/pause, Shift+Left/Right = -50/+50.\n"
    ...
    def on_load_models(self):
        ...
        if not d: return
        self.models_dir = Path(d); self.model_to_names.clear()
        count = 0
        for yaml_path in list(self.models_dir.glob("*.yaml")) + list(self.models_dir.glob("*.yml")):
            try:
        ...
        if not self.model_to_names:
            ...
            return
        self.status.set(f"Loaded {count} model(s).")
        self._update_transport_enabled()
    ...
    def toggle_play(self):
        ...
        if self.playing:
            ...
            self._render_from_cache(draw_boxes=True)  # show boxes when paused
        else:
            self._cancel_play_timer()
    ...
    def _freeze_at_end(self):
        """Freeze cleanly on the last decodable frame; do not rewind."""
        # Try the reported last frame, step back a few if needed
        target = self.total_frames - 1 if self.total_frames > 0 else self.frame_idx
    ...
    def _play_loop(self):
        ...
        for _ in range(self.play_step):
            ...
            last = frame
        if last is not None:
            # Update current frame index from decoder position (next-to-read minus 1)
            pos = int(self.cap.get(cv2.CAP_PROP_POS_FRAMES))
            self.frame_idx = max(0, pos - 1)
            self._show_frame(last, draw_boxes=False)  # hide boxes while playing
    ...
    def step_seek(self, delta: int):
        ...
        new_idx = clamp((self.frame_idx if self.frame_idx >= 0 else 0) + delta, 0, max(0, (self.total_frames or 1) - 1))
        self._seek_and_show(new_idx)
        self.end_reached = False
    ...
    def _redraw_boxes(self):
        """Draw boxes for current frame (if any)."""
    ...
    def _open_assign_dialog(self, x1, y1, x2, y2):
        ...
        def do_save():
            ...
            self.annotations.append(ann)  # stored under current frame in ann_by_frame
            self._render_from_cache(draw_boxes=True)
            self._refresh_lists()
            dlg.destroy()
